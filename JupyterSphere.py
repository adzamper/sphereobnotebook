...
# Includes the pre-factor in equation 21b
    ...
    def dh_tot_step(mtx, dipole_m, rtx, rsp, mu, sigma_ob, thick_ob, t, o, sigma_sp, a, T):
        ...
        def my_function_x(x):
            return -dh_obdt_xyz(mtx, dipole_m, rtx, rsp, x, mu, sigma_ob,
                                thick_ob)[0] * \
        ...
        return np.array(
            [resultx + (ob_array[0] * thetaz), resultz + (ob_array[1] * thetaz), resulty + (ob_array[2] * thetaz)])
...
# Purpose: this function calculates the sphere-overburden response in the first order, no wave
    ...
    def h_total_step_1storder(
        ...
        temp = (2 * math.pi * (a ** 3)) * dh_tot_step(
            mtx, dipole_m, [0, 0, rtx[2]], [-rtx[0], -rtx[1], rsp[2]], mu, sigma_ob,
            thick_ob, t, 0, sigma_sp, a, T)

        # store sphere moment
        msp = np.array([temp[0], temp[2], temp[1]])
        # msp = np.array([(2 * math.pi * (a ** 3)) * temp[0], (2 * math.pi * (a ** 3)) * temp[2], (2 * math.pi * (a ** 3)) * temp[1]])
...
def SphereWidget():
    i = interact(
        ...
        sigob=FloatSlider(min=0.001, max=5.0, step=.01, value=0.03, continuous_update=False,
                          description="$\sigma_{ob}$"),

        sigsp=FloatSlider(min=0.01, max=10.0, step=.01, value=0.5, continuous_update=False,
                          description="$\sigma_{sp}$"),
